  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        "https://achieversit.com/management/captureLeadRequest",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        setError("Submission failed. Please try again."); // Show error if submission fails
      }
    } catch (error) {
  ...
  {error && <p className="mt-2 text-sm text-maincolor_1">{error}</p>}
        ...
        <div className="footer-end py-4 px-0 lg:mx-45">
          <div className="text-center">
            <h4 className="text-lg font-medium text-2xl">Disclaimer</h4>
